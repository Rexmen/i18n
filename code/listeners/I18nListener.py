...
class I18nListener:
    ...
    def __init__(self, locale='en-US', not_show_warning_words='None'):
        ...
        for f in glob('%s/languageFiles/%s/*%s.json' % (os.path.dirname(os.path.abspath(__file__)), locale, locale)): #?ÑÁ®ÆË™ûË?ÁøªË≠ØÊ™?(common-xx.json more-xx.json etc.)
            with open(f, 'r', encoding='UTF-8') as i18n_file: #Â∞ájsonÊ™îÊ???
                i18n_dict = json.load(i18n_file)  #Â∞áË??≠Á?jsonÁµêÊ?ËΩâÊ??êpython dict
            self.combine_i18n_dict(source_dict=i18n_dict, target_dict=I18nListener.TRANSLATION_FILE)
        I18nListener.MAP = I18nMap(I18nListener.TRANSLATION_FILE, locale) #i18nMAPÔºåÂÇ≥?•Ê??âÁøªË≠ØÊ?dict?ÑTRANSLATION_FILE ?? Ë™ûË?
    ...
    '''
    def combine_i18n_dict(self, source_dict, target_dict): #ÂØ´‰?‰ªΩÂà∞TRANSLATION_FILE
